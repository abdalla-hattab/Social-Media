...
window.openCreatePostModal = function(postId = null) {
    ...
    if (createPostModal) {
            ...
            if (activeBoard && activeBoard.cards) {
                ...
                if (post) {
                    if (textArea) {
                        ...
                        if (instagramArea) {
                            instagramArea.value = typeof post.fullTextInstagram !== 'undefined' ? post.fullTextInstagram : (post.fullText || post.description || '');
                        }
                        ...
                        if (previewContainer && gallery) {
                            ...
                            mediaItems.forEach((mi, index) => {
                                ...
                                if (mi.type === 'frame-io') {
                                    ...
                                    wrap.setAttribute('data-url', mi.url);
                                    wrap.style.cssText = "position: relative; border-radius: 6px; overflow: hidden; display: inline-flex; flex-shrink: 0; margin-bottom: 8px;";
                                    
                                    wrap.innerHTML = `
                                        <button onclick="event.stopPropagation(); window.open('${mi.url}', '_blank')" style="display:flex; align-items:center; justify-content:center; gap:6px; background: #10b981; color: white; border: none; border-radius: 6px; padding: 10px 16px; padding-right: 36px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; white-space: nowrap;">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>
                                            عرض المنتج
                                        </button>
                                        ${window.isClientView ? '' : `<button onclick="event.stopPropagation(); window.removeMediaItem(this)" style="position:absolute; top: 50%; right: 8px; transform: translateY(-50%); width:20px; height:20px; border-radius:50%; background:rgba(255,255,255,0.3); border:none; display:flex; align-items:center; justify-content:center; cursor:pointer; color:white; font-weight:bold; font-size:14px; z-index:10; line-height: 1; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.5)'" onmouseout="this.style.background='rgba(255,255,255,0.3)'">×</button>`}
                                    `;
                                } else {
            ...
            if (activeBoard && activeBoard.cards) {
                ...
                if (dayPosts.length > 0 || postId) {
                    ...
                    html += dayPosts.map((p, idx) => {
                        ...
                        const defaultText = window.shareType === 'script_plan' ? 'عرض السكربت' : (window.shareType === 'publishing_plan' ? 'عرض التصميم ومحتوى النشر' : 'مسودة منشور...');
        ...
        if (modalBody) {
            ...
            const wrapFb = document.getElementById('post-content-wrap');
...
window.onMonthClick = function(e, idx) {
    ...
    if (!window.activeSocialMonthView) {
        window.activeSocialMonthView = { year: new Date().getFullYear(), month: idx };
    } else {
        window.activeSocialMonthView.month = idx;
    }
    ...
    plusBtn.onclick = (ev) => {
        ...
        updateMonthBadge(getCircle(), currentQty);
    };
    ...
    plusBtn2.onclick = (ev) => {
        ...
        updateMonthBadge2(getCircle(), currentQty2);
    };
    ...
    plusBtn3.onclick = (ev) => {
        ...
        updateMonthBadge3(getCircle(), currentQty3);
    };
...
window.generatePipelineHtml = function(board) {
    ...
    if (!window.isClientView) {
        contractHtml = `
            <div onclick="window.editContract(${curY}, ${curM})" style="cursor: pointer; display: inline-flex; flex-direction: column; align-items: center; justify-content: center; transition: all 0.2s; transform-origin: center; flex-shrink: 0;" onmouseover="this.style.transform='scale(1.02)'; this.style.opacity='0.9'" onmouseout="this.style.transform='scale(1)'; this.style.opacity='1'">
                ...
                <span style="font-size: 14px; font-weight: 600; color: #475569; display: inline-flex; align-items: center; gap: 8px; background: #eff6ff; padding: 6px 14px; border-radius: 20px; border: 1px solid #bfdbfe; white-space: nowrap;">
    ...
    monthNamesAll.forEach((m, idx) => {
        ...
        if (isStart) {
            let suffix = 'th';
            const j = contractStartDay % 10;
            const k = contractStartDay % 100;
            if (j === 1 && k !== 11) suffix = 'st';
            else if (j === 2 && k !== 12) suffix = 'nd';
            else if (j === 3 && k !== 13) suffix = 'rd';
            
            contentHtml = `
                <span style="font-size: 16px; font-weight: 900; color: ${color}; margin-bottom: 2px;">${contractStartDay}<span style="font-size: 11px;">${suffix}</span></span>
...
function renderSocialSchedulerApp(activeBoard) {
    ...
    const topRowHtml = `
        <div class="sm-header-banner" style="margin-bottom: 24px; display: flex; flex-direction: column; align-items: flex-start; gap: 16px;">
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; width: 100%; padding: 6px; border-radius: 8px; outline: none; box-shadow: none;">
                ...
                <div style="display: flex; gap: 8px;">
                    <!-- Description Button -->
                    <a href="https://description.masaratkobra.com" target="_blank" style="display: flex; align-items: center; gap: 8px; background: #673de6; color: white; padding: 8px 16px; border-radius: 8px; font-weight: 700; text-decoration: none; font-size: 14px; border: 1px solid #5a35cc; box-shadow: 0 4px 6px rgba(0,0,0,0.1); transition: all 0.2s;" onmouseover="this.style.background='#5a35cc'; this.style.transform='translateY(-1px)'" onmouseout="this.style.background='#673de6'; this.style.transform='translateY(0)'" title="الذهاب إلى Description">
                    ...
                    <a href="https://trello.com/b/PQ3Zdn0R/ads-value-stream-a-system-for-every-client-to-make-monthly-20000-sar" target="_blank" style="display: flex; align-items: center; gap: 8px; background: #1e3252; color: white; padding: 8px 16px; border-radius: 8px; font-weight: 700; text-decoration: none; font-size: 14px; border: 1px solid #162640; box-shadow: 0 4px 6px rgba(0,0,0,0.1); transition: all 0.2s;" onmouseover="this.style.background='#162640'; this.style.transform='translateY(-1px)'" onmouseout="this.style.background='#1e3252'; this.style.transform='translateY(0)'" title="الذهاب إلى Ads System">
    ...
    window.updateLiveDiff = function() {
        ...
        const instagramArea = document.querySelector('.sm-textarea-instagram');
        const currentInstagramText = instagramArea ? instagramArea.value : (post.fullTextInstagram || currentText);
                ...
                sortedDates.forEach(dateStr => {
                    ...
                    postsByDate[dateStr].forEach(p => {
                        ...
                        <div id="mobile-post-card-${p.id}" class="sm-feed-post-card" style="position: relative; padding: 14px 16px; border-radius: 16px; background: ${bg}; border: ${border}; color: #1e293b; display: flex; flex-direction: column; box-shadow: 0 2px 6px rgba(0,0,0,0.02); transition: all 0.2s ease; direction: rtl; width: 100%; box-sizing: border-box; margin-bottom: 12px;" onmouseover="this.style.boxShadow='0 6px 16px rgba(0,0,0,0.06)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.boxShadow='0 2px 6px rgba(0,0,0,0.02)'; this.style.transform='none';">
                            <div style="display:flex; align-items:center; justify-content:${(postFrameIoLink && window.shareType !== 'publishing_plan' && window.shareType !== 'content_plan') ? 'space-between' : 'center'}; width: 100%;">
                    ...
                    <div class="sm-sidebar-header" style="display:flex; align-items:flex-start; justify-content:space-between; border-bottom:2px solid #f1f5f9; padding-bottom:20px; margin-bottom:20px;">
                        ...
                        <div class="sm-selected-date-text" style="display:flex; flex-direction:column; gap:4px; align-items:flex-start; text-align:right;">
                            ...
                            <div style="display:flex; gap:8px; margin-top:6px;">
                                ${['facebook', 'instagram', 'twitter', 'linkedin', 'tiktok', 'snapchat'].map(platform => {
                                    const colors = {facebook: '#0ea5e9', instagram: '#ec4899', twitter: '#0f172a', linkedin: '#2563eb', tiktok: '#18181b', snapchat: '#ca8a04'};
                                    const bgs = {facebook: '#e0f2fe', instagram: '#fce7f3', twitter: '#f1f5f9', linkedin: '#dbeafe', tiktok: '#f4f4f5', snapchat: '#fef9c3'};
                                    const svgs = {
                                    ...
                                    if (hasLink) {
                                        ...
                                        actionBtnHtml = `<div onclick="window.smHandleSocialActionClick(event, '${platform}', false, '${storageKey}', '')" style="width:16px; height:16px; border-radius:50%; background:#fef3c7; border:1px solid #fcd34d; display:flex; align-items:center; justify-content:center; cursor:pointer; margin-top:-6px; z-index:2; position:relative; font-size:10px; font-weight:bold;" title="إضافة رابط">
                                            ⚠️
                                        </div>`;
                                    }

                                    return `
                                        <div style="display:flex; flex-direction:column; align-items:center; transition:all 0.2s ease;" onmouseover="this.style.transform='scale(1.1)';" onmouseout="this.style.transform='scale(1)';">
                                            <div onclick="window.handleSocialIconClick('${platform}', this)" oncontextmenu="window.handleSocialIconContextMenu(event, '${platform}')" style="display:flex; align-items:center; justify-content:center; width:30px; height:30px; background:${bgs[platform]}; border:2px solid ${window.activePreviewPlatform === platform ? colors[platform] : 'transparent'}; border-radius:50%; color:${colors[platform]}; cursor:pointer; position:relative; z-index:1;">
                                                ${svgs[platform]}
                                            </div>
                                            ${actionBtnHtml}
                                        </div>
    ...
    window.smHandleSocialActionClick = function(event, platform, hasLink, storageKey, savedLink) {
        event.stopPropagation();
    ...
    window.handleSocialIconClick = function(platform, element) {
        if (window.activePreviewPlatform === platform) {
            window.activePreviewPlatform = null;
            element.parentElement.parentElement.querySelectorAll('div[onclick*="handleSocialIconClick"]').forEach(el => el.style.border = '2px solid transparent');
        } else {
...
window.saveSocialDraft = async function(isAutoSave = false) {
    try {
        ...
        const isClientModified = window.isClientView 
            ? (clientEdits !== '' || (existingPost && existingPost.fullText !== textContent) || (existingPost && existingPost.fullTextInstagram !== instagramContent) || (existingPost && existingPost.fullTextSnapchat !== snapchatContent) || (existingPost && existingPost.fullTextTiktok !== tiktokContent) || (existingPost && existingPost.postType !== postType) || (existingPost && typeof existingPost.clientModified !== 'undefined' ? existingPost.clientModified : false))
        ...
        if (window.isClientView && isClientModified && existingPost && !originalState) {
            originalState = {
                ...
                fullTextInstagram: existingPost.fullTextInstagram || existingPost.fullText,
...
window.requestClientEdit = function(postId) {
    if (typeof window.openCreatePostModal === 'function') {
        window.openCreatePostModal(postId);
        setTimeout(() => {
            const input = document.getElementById('clientEditsInput');
            if (input) {
                input.focus();
